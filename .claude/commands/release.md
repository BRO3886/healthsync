Create a new release for the `healthsync` project.
...
This produces archives in `bin/`:
- `healthsync-darwin-arm64.tar.gz`
- `healthsync-darwin-amd64.tar.gz`
- `healthsync-linux-arm64.tar.gz`
- `healthsync-linux-amd64.tar.gz`
- `healthsync-windows-arm64.zip`
- `healthsync-windows-amd64.zip`

Verify the version is correct before uploading:

```bash
cd /tmp && tar -xzf /path/to/bin/healthsync-darwin-arm64.tar.gz && ./healthsync version && rm ./healthsync
```

The output must show exactly `healthsync v<VERSION>` with no commit suffix.
...
gh release create v<VERSION> \
  bin/healthsync-darwin-arm64.tar.gz \
  bin/healthsync-darwin-amd64.tar.gz \
  bin/healthsync-linux-arm64.tar.gz \
  bin/healthsync-linux-amd64.tar.gz \
  bin/healthsync-windows-arm64.zip \
  bin/healthsync-windows-amd64.zip \
  --title "v<VERSION>" \
  --notes "$(cat <<'EOF'
<RELEASE_NOTES>
EOF
...
```bash
go install github.com/BRO3886/healthsync@v<VERSION>
```

**Full Changelog**: https://github.com/BRO3886/healthsync/compare/v<PREV>...v<VERSION>
```
