...
const skillDirName = "healthsync"
const versionFileName = ".healthsync-version"
...
func Install(embeddedFS fs.FS, target AgentTarget, version string) ([]string, error) {
	...
	err := fs.WalkDir(embeddedFS, "skills/healthsync", func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}

		// Convert embedded path to destination path
		// path is like "skills/healthsync/SKILL.md" -> "SKILL.md"
		relPath, err := filepath.Rel("skills/healthsync", path)
		if err != nil {
			return err
		}
